...
// Writes a meta.js describing the current build, a meta.d.ts typing it, and a
// meta.json alongside them when asked. Every path here resolves against the current working directory,
...
// The declaration file that rides along with the script. Its fields come from
// the same object, so it cannot promise a key the script does not emit: a
// package.json with no version produces types with no version either.
//
// A global script rather than a module, because a single top level import or
// export would stop the Window augmentation from being global and the typing
// would silently apply to nothing. Every value is a string.
//
// BuildMeta is a type alias, per the repo's no-interfaces rule. Window has to
// stay an interface: augmenting the DOM's Window is declaration merging, and
// only interfaces merge. `type Window` is a duplicate identifier against
// lib.dom.d.ts instead, which fails the consumer's own typecheck.
const fields = Object.entries(meta)
  .filter(([, value]) => value !== undefined)
  .map(([key]) => `  ${key}: string;`)
  .join('\n');

const declaration = `// Generated by build-meta. Describes the object meta.js installs on the global.
type BuildMeta = {
${fields}
};

interface Window {
  ${JSON.stringify(GLOBAL_KEY)}?: BuildMeta;
}
`;
...
writeFileSync(resolve(flags['src-folder'], 'meta.d.ts'), declaration);
