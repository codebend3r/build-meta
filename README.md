...
The key is optional because nothing guarantees the script has loaded by the time your code reads it. The interface describes the object that was actually emitted, so a `package.json` with no `version` produces types with no `version` either, rather than promising a field that is not there.
